 ...
 * 3. 仅保留一个「节点选择」select 组，直接平铺全部过滤后的自建节点
...
// Bettbox 专用版：不启用 Bettbox 对 ruleOptionsEnable 的可视化接管，避免自定义策略组被 UI 适配层裁剪。
const Compatible_With_Bettbox = { ruleOptionsEnable: false };
...
function filterAndNormalizeProxies(config) {
  ...
  if (!filteredProxies.length) {
    throw new Error('配置文件中未找到任何自建代理节点，请检查当前配置是否包含 proxies');
  }
