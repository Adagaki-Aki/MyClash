 ...
 * 7. 保留 MyClash 的 DNS / Hosts / 私有 DNS / Hosts -> proxy.server 改写逻辑
 * 8. 保留 MyClash 的国内外规则、服务 Rule Providers；默认不拦截国外 QUIC
 *
 * 重要：本版不创建 url-test / load-balance，不做自动测速，不做自动故障转移。
 * 9. 支持 proxy-providers；默认启用 TUN / DNS 劫持，关闭 NTP / LAN / IPv6，保留规则模式、节点选择持久化和本地 API。
 */
...
const ruleOptionsEnable = {
  ...
  // MyClash 原有非分流功能
  过滤非地区节点: true,
  // 修复速度问题：默认不拦截国外 UDP/443（QUIC / HTTP/3）
  屏蔽国外QUIC: false,
  代理IPV4优先: false,
  代理IPV6优先: false,
  链式代理: false,
...
function buildConfig(config, inheritOriginalConfig) {
  ...
  // 修复原机场配置被脚本关闭 TUN 的问题：
  // 优先继承机场原 TUN 参数；若原配置没有 TUN，则使用安全默认值。
  const originalTun =
    inheritOriginalConfig && config.tun && typeof config.tun === 'object' ? config.tun : {};
  newConfig.tun = {
    ...originalTun,
    enable: true,
    'dns-hijack':
      Array.isArray(originalTun['dns-hijack']) && originalTun['dns-hijack'].length > 0
        ? originalTun['dns-hijack']
        : ['any:53'],
    'auto-route': originalTun['auto-route'] !== false,
    'auto-detect-interface': originalTun['auto-detect-interface'] !== false,
  };
